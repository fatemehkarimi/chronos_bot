...
func (repo *PostgresRepository) AddFeatureFlag(ownerId int, featureFlag string) error {
	query := `INSERT INTO feature_flag(owner_id, feature_flag) VALUES ($1, $2);`
	_, err := repo.DB.Exec(query, ownerId, featureFlag)
	return err
}
...
func (repo *PostgresRepository) AddSchedule(featureFlag, value string, calendarType CalendarType, year, month, day, hour, minute int) (int, error) {
	query := `
	INSERT INTO schedule(feature_flag, value, calendar_type, year, month, day, hour, minute) VALUES ($1, $2, $3, $4, $5, $6, $7, $8)`
	var scheduleId int
	err := repo.DB.QueryRow(query).Scan(&scheduleId)
